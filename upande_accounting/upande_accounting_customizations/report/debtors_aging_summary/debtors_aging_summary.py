...
Payment Ledger Entry) and layers in two additions:
  ...
  3. Custom column layout — Credit Note / Outstanding / Invoiced Amount sit
     after the ageing buckets, just before Territory / Customer Group; the
     base report's "<0" (not-yet-due) ageing column is dropped.
...
class DebtorsAgingReport(AccountsReceivableSummary):
    ...
    def get_columns(self):
        ...
        # buckets, right before Territory / Customer Group. Paid Amount only
        # makes sense once advance payments are in view, so it's gated on
